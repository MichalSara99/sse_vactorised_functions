#pragma once
#if !defined(_SPECIAL_FUNCTIONS_T)
#define _SPECIAL_FUNCTIONS_T
...
using packed_sse::erf_sse_packed;
using packed_sse::erfc_sse_packed;
...
void testErfSSEDouble() {
	...
	bool rc1 = erf_sse_packed(x, n, res1);
...
void testErfcSSEDouble() {
	...
	auto start_asm = std::chrono::system_clock::now();
	bool rc1 = erfc_sse_packed(x, n, res1);
	auto end_asm = std::chrono::system_clock::now();
	auto elapsed_asm = std::chrono::duration<double>(end_asm - start_asm).count();
